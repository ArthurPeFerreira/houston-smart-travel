...
export default function UserTable() {
  ...
  const [loadingCreateUserModal, setLoadingCreateUserModal] = useState<boolean>(false);
  const [showEditUserModal, setShowEditUserModal] = useState<boolean>(false);
  const [loadingEditUserModal, setLoadingEditUserModal] = useState<boolean>(false);
  ...
  async function handleCreateUser(userData: CreateUserType) {
    setLoadingCreateUserModal(true);
    try {
    ...
    setLoadingCreateUserModal(false);
  ...
  async function handleEditUser(userData: EditUserType, userId: number) {
    ...
    try {
      ...
      const userEditedData = await api.put(`api/admin/user/${userId}`, userData);
      ...
      <div className="bg-gray-800 w-full p-5 rounded-2xl flex flex-col items-center justify-center text-white">
        ...
        {users ? (
          <div className="w-full flex flex-col items-center justify-center text-white">      
            ...
            <table className="min-w-full border-collapse text-sm md:text-lg">
              <thead>
                <tr className="bg-gray-700">
                  ...
                  <th className={`${classItens} hidden sm:table-cell`}>Last Login</th>
                ...
                {users.map((user) => (
                  <tr key={user.id} className="text-center bg-gray-600">
                    ...
                    <td className={`${classItens} hidden sm:table-cell`}>
                      {user.lastLogin ? convertToHoustonTime(String(user.lastLogin)) : "Never"}
                    </td>
                    <td className={classItens}>
                      <button onClick={() => { setShowEditUserModal(true); setUserToEdit(user); }} className="bg-yellow-500 py-2 px-5 rounded cursor-pointer hover:bg-yellow-600">Edit</button>
                    </td>
                    <td className={classItens}>
                      <button onClick={() => handleDeleteUser(user.id)} className="bg-red-500 py-2 px-3 rounded cursor-pointer hover:bg-red-600">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <CreateUserModal isOpen={showCreateUserModal} isLoading={loadingCreateUserModal} onClose={() => setShowCreateUserModal(false)} onCreateUser={handleCreateUser} />
            <EditUserModal isOpen={showEditUserModal} isLoading={loadingEditUserModal} userInfo={userToEdit} onClose={() => setShowEditUserModal(false)} onEditUser={handleEditUser} />
          </div>
        ) : (
